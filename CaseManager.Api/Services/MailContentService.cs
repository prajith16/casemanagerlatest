...
/// <summary>
/// Service implementation for MailContent operations
/// </summary>
public class MailContentService : IMailContentService
{
    private readonly IRepository<MailContent> _repository;
    ...
    public MailContentService(
        IRepository<MailContent> repository,
        ...
        IConfiguration configuration)
    {
        ...
        // Load PDF content once during initialization
        _pdfContent = LoadPdfContent();
    }

    public async Task<IEnumerable<MailContent>> GetAllMailContentsAsync()
    {
        return await _repository.GetAllAsync();
    }

    public async Task<MailContent?> GetMailContentByIdAsync(int id)
    {
        return await _repository.GetByIdAsync(id);
    }

    public async Task<MailContent> CreateMailContentAsync(MailContent mailContent)
    {
        return await _repository.AddAsync(mailContent);
    }

    public async Task<MailContent> UpdateMailContentAsync(MailContent mailContent)
    {
        return await _repository.UpdateAsync(mailContent);
    }

    public async Task<bool> DeleteMailContentAsync(int id)
    {
    ...
    private async Task<string> GenerateAIResponseAsync(MailContent mailContent)
    {
        ...
        // Try GitHub Models configuration first
        var githubApiKey = _configuration["GitHubModels:ApiKey"];
        var githubModelId = _configuration["GitHubModels:ModelId"] ?? "gpt-4o";
        ...
        else
        {
            ...
            var azureDeployment = _configuration["AzureOpenAI:DeploymentName"];
